-- ============================================
-- Instagram Outreach A/B Testing - Database Schema
...
RETURNS TRIGGER AS $$
BEGIN
    -- Обновляем дневную статистику только если есть вариант шаблона
    IF NEW.template_variant IS NOT NULL THEN
        -- Обновляем при отправке сообщения
        ...
        IF TG_OP = 'UPDATE' AND NEW.opened_message = true AND (OLD.opened_message IS NULL OR OLD.opened_message = false) THEN
            INSERT INTO outreach_ab_test_stats (
                template_group,
                variant_name,
                date,
        ...
        IF TG_OP = 'UPDATE' AND NEW.outreach_status = 'REPLIED' AND (OLD.outreach_status IS NULL OR OLD.outreach_status != 'REPLIED') THEN
            INSERT INTO outreach_ab_test_stats (
                template_group,
                variant_name,
                date,
        ...
        IF TG_OP = 'UPDATE' AND NEW.outreach_status = 'INTERESTED' AND (OLD.outreach_status IS NULL OR OLD.outreach_status != 'INTERESTED') THEN
            INSERT INTO outreach_ab_test_stats (
                template_group,
                variant_name,
                date,
        ...
        IF TG_OP = 'UPDATE' AND NEW.outreach_status = 'CLOSED' AND (OLD.outreach_status IS NULL OR OLD.outreach_status != 'CLOSED') THEN
            INSERT INTO outreach_ab_test_stats (
                template_group,
                variant_name,
                date,
...
-- 9. Вставляем начальные варианты шаблонов (можно будет обновить через API/бот)
