# 🤖 DevOps Agent для LoyalityBot

Автоматизированный агент для управления деплоями, мониторинга и обслуживания ботов.

## 🚀 Быстрый старт

```bash
# Сделать скрипт исполняемым (если еще не сделано)
chmod +x devops_agent.py

# Показать справку
./devops_agent.py --help

# Проверить статус всех ботов
./devops_agent.py status

# Показать информацию о проекте
./devops_agent.py info
```

## 📋 Доступные команды

### `status` - Проверка статуса ботов

Проверяет статус ботов на Fly.io:

```bash
# Проверить все боты
./devops_agent.py status

# Проверить конкретный бот
./devops_agent.py status --bot admin
./devops_agent.py status --bot partner
./devops_agent.py status --bot client
```

### `deploy` - Деплой ботов

Деплоит ботов на Fly.io:

```bash
# Задеплоить все боты
./devops_agent.py deploy

# Задеплоить конкретный бот
./devops_agent.py deploy --bot admin

# Локальная сборка (без --remote-only)
./devops_agent.py deploy --no-remote
```

### `logs` - Просмотр логов

Просматривает логи бота:

```bash
# Последние 50 строк логов
./devops_agent.py logs admin

# Последние 100 строк
./devops_agent.py logs partner --lines 100

# Логи клиентского бота
./devops_agent.py logs client
```

### `restart` - Перезапуск ботов

Перезапускает ботов:

```bash
# Перезапустить все боты
./devops_agent.py restart

# Перезапустить конкретный бот
./devops_agent.py restart --bot admin
```

### `health` - Проверка здоровья

Проверяет здоровье всех сервисов:

```bash
./devops_agent.py health
```

Показывает:
- Статус всех ботов
- Наличие конфигурационных файлов
- Наличие скриптов ботов
- Доступность Fly.io CLI

### `monitor` - Мониторинг

Мониторит статус ботов с заданным интервалом:

```bash
# Мониторинг с интервалом 60 секунд на 5 минут
./devops_agent.py monitor

# Кастомный интервал и длительность
./devops_agent.py monitor --interval 30 --duration 600
```

### `secrets` - Управление секретами

Устанавливает секреты из `.env` файла:

```bash
# Установить секреты для всех ботов
./devops_agent.py secrets

# Установить секреты для конкретного бота
./devops_agent.py secrets --bot admin

# Использовать другой файл с переменными
./devops_agent.py secrets --env .env.production
```

### `info` - Информация о проекте

Показывает информацию о проекте:

```bash
./devops_agent.py info
```

## 🔧 Требования

- Python 3.7+
- Fly.io CLI (`flyctl`) - для работы с Fly.io
  ```bash
  # Установка на macOS
  brew install flyctl
  
  # Установка на Linux/Windows
  curl -L https://fly.io/install.sh | sh
  ```

## 📦 Структура ботов

Агент управляет тремя ботами:

1. **admin** - Админ-бот (`loyalitybot-admin`)
   - Конфиг: `fly.admin.toml`
   - Скрипт: `admin_bot.py`

2. **partner** - Партнерский бот (`loyalitybot-partner`)
   - Конфиг: `fly.partner.toml`
   - Скрипт: `bot.py`

3. **client** - Клиентский бот (`loyalitybot-client`)
   - Конфиг: `fly.client.toml`
   - Скрипт: `client_handler.py`

## 🎯 Примеры использования

### Ежедневная проверка

```bash
# Проверить статус всех ботов
./devops_agent.py status

# Проверить здоровье системы
./devops_agent.py health
```

### Деплой после изменений

```bash
# Задеплоить все боты
./devops_agent.py deploy

# Проверить статус после деплоя
./devops_agent.py status
```

### Отладка проблем

```bash
# Проверить статус
./devops_agent.py status

# Посмотреть логи проблемного бота
./devops_agent.py logs admin --lines 200

# Перезапустить бота
./devops_agent.py restart --bot admin
```

### Мониторинг после деплоя

```bash
# Мониторить статус 10 минут с интервалом 30 секунд
./devops_agent.py monitor --interval 30 --duration 600
```

## 🔐 Управление секретами

Перед первым деплоем необходимо установить секреты:

```bash
# Убедитесь, что .env файл содержит все необходимые переменные
cat .env

# Установите секреты для всех ботов
./devops_agent.py secrets
```

## 🐛 Решение проблем

### Fly.io CLI не найден

```bash
# Установите flyctl
brew install flyctl  # macOS
# или
curl -L https://fly.io/install.sh | sh

# Проверьте установку
flyctl --version
```

### Бот не найден на Fly.io

Если бот не найден, сначала создайте приложение:

```bash
# Создать приложение для админ-бота
flyctl launch --config fly.admin.toml --app loyalitybot-admin

# Затем установите секреты
./devops_agent.py secrets --bot admin

# И задеплойте
./devops_agent.py deploy --bot admin
```

### Ошибки при деплое

1. Проверьте, что вы авторизованы в Fly.io:
   ```bash
   flyctl auth whoami
   ```

2. Проверьте наличие конфигурационных файлов:
   ```bash
   ./devops_agent.py info
   ```

3. Проверьте логи:
   ```bash
   ./devops_agent.py logs admin
   ```

## 📝 Интеграция с CI/CD

Агент можно использовать в GitHub Actions:

```yaml
- name: Check bot status
  run: |
    python3 devops_agent.py status

- name: Deploy bots
  run: |
    python3 devops_agent.py deploy
  env:
    FLY_API_TOKEN: ${{ secrets.FLY_API_TOKEN }}
```

## 🔄 Автоматизация

Можно создать алиасы в `.bashrc` или `.zshrc`:

```bash
# Добавить в ~/.zshrc или ~/.bashrc
alias bot-status='cd /path/to/loyalitybot && ./devops_agent.py status'
alias bot-deploy='cd /path/to/loyalitybot && ./devops_agent.py deploy'
alias bot-logs='cd /path/to/loyalitybot && ./devops_agent.py logs'
```

## 📊 Расширение функциональности

Агент можно расширить для:
- Интеграции с другими платформами (Render, Railway)
- Мониторинга метрик (CPU, память)
- Автоматических алертов
- Бэкапов конфигураций
- Автоматического масштабирования

## 📞 Поддержка

При возникновении проблем:
1. Проверьте логи: `./devops_agent.py logs <bot>`
2. Проверьте здоровье: `./devops_agent.py health`
3. Проверьте статус: `./devops_agent.py status`

---

**Версия:** 1.0  
**Последнее обновление:** Декабрь 2024




