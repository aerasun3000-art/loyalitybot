# 📝 Пошаговая настройка Sentry Webhook

## ⚡ Быстрый способ (5 минут)

### Шаг 1: Откройте Sentry Dashboard

1. Перейдите на https://sentry.io/
2. Войдите в аккаунт
3. Выберите ваш проект (или создайте новый)

**URL проекта будет выглядеть так:**
```
https://sentry.io/organizations/YOUR-ORG/projects/YOUR-PROJECT/
```

Запомните:
- `YOUR-ORG` - это Organization Slug
- `YOUR-PROJECT` - это Project Slug (обычно "python")

---

### Шаг 2: Настройте Webhook Integration

1. В левом меню нажмите **⚙️ Settings**
2. В боковом меню выберите **Integrations**
3. Найдите **Webhooks** в списке интеграций
4. Нажмите **Configure** или **Add to Project**

---

### Шаг 3: Заполните форму Webhook

**Callback URL:**
```
https://your-domain.com/api/sentry-webhook
```

**Для тестирования (используйте ngrok):**
```bash
# В терминале запустите:
ngrok http 8003

# Скопируйте HTTPS URL (например: https://abc123.ngrok.io)
# Используйте: https://abc123.ngrok.io/api/sentry-webhook
```

**Secret (опционально):**
- Скопируйте значение из `.env`:
  ```bash
  grep SENTRY_WEBHOOK_SECRET .env
  ```
- Или оставьте пустым

5. Нажмите **Save Changes**

---

### Шаг 4: Создайте Alert Rule

1. В левом меню выберите **Alerts**
2. Нажмите **Create Alert** (или **+ Create Alert Rule**)
3. Выберите тип: **Issues**

**Настройте условия (When):**
- ✅ Отметьте: **"An issue is first seen"**
- ✅ Отметьте: **"An issue changes state from resolved to unresolved"**
- ✅ Отметьте: **"The issue is seen more than 10 times in 1 minute"** (опционально)

**Настройте действия (Then):**
- В разделе **"Then perform these actions"**
- Выберите **"Send a notification via Webhooks"**
- Выберите ваш настроенный webhook из списка

4. Нажмите **Save Alert Rule**

---

### Шаг 5: Тестирование

#### Вариант A: Через API (уже настроено)

```bash
# Вызовите тестовую ошибку
curl http://127.0.0.1:8003/sentry-debug

# Проверьте:
# 1. Sentry Dashboard → Issues - должна появиться ошибка
# 2. Telegram - должно прийти уведомление
```

#### Вариант B: Через Sentry UI

1. В Sentry Dashboard → **Settings** → **Integrations** → **Webhooks**
2. Найдите ваш webhook
3. Нажмите кнопку **"Test"** или **"Send Test"**
4. Проверьте Telegram

---

## 🔍 Проверка что всё работает

### 1. Проверка webhook endpoint

```bash
curl -X POST http://127.0.0.1:8003/api/sentry-webhook \
  -H "Content-Type: application/json" \
  -d '{"test": "data"}'

# Должен вернуть:
# {"status":"ok","message":"Alert sent to Telegram"}
```

### 2. Проверка Telegram

- Откройте Telegram
- Найдите бота (используйте токен из переменной окружения `SENTRY_ALERT_TELEGRAM_TOKEN`)
- Проверьте что приходят уведомления

### 3. Проверка логов

```bash
# Логи secure_api
tail -f logs/secure_api.log

# Должны появиться записи о получении webhook
```

---

## 🐛 Если что-то не работает

### Webhook не получает запросы

1. **Проверьте URL:**
   - Должен быть доступен из интернета
   - Для теста используйте ngrok
   - Формат: `https://domain.com/api/sentry-webhook`

2. **Проверьте что сервер запущен:**
   ```bash
   curl http://127.0.0.1:8003/health
   ```

3. **Проверьте логи:**
   ```bash
   tail -f logs/secure_api.log
   ```

### Уведомления не приходят в Telegram

1. **Проверьте переменные:**
   ```bash
   grep SENTRY_ALERT .env
   ```
   Должны быть:
   - `SENTRY_ALERT_TELEGRAM_TOKEN=YOUR_BOT_TOKEN_HERE`
   - `SENTRY_ALERT_CHAT_ID=YOUR_CHAT_ID_HERE`

2. **Проверьте что бот запущен:**
   - Бот должен быть запущен и отвечать на команды

3. **Протестируйте отправку вручную:**
   ```bash
   curl -X POST "https://api.telegram.org/botYOUR_BOT_TOKEN_HERE/sendMessage" \
     -d "chat_id=YOUR_CHAT_ID_HERE&text=Test message"
   ```

### Ошибка 401 (Invalid signature)

- Убедитесь что `SENTRY_WEBHOOK_SECRET` одинаковый в Sentry и `.env`
- Или отключите проверку (оставьте Secret пустым в Sentry)

---

## ✅ Готово!

После настройки все критические ошибки будут автоматически отправляться в Telegram!

**Формат уведомления:**
```
❌ SENTRY ALERT

ERROR: ZeroDivisionError: division by zero

📍 Location: secure_api.py:155 in trigger_error

🌍 Environment: production
📦 Release: loyaltybot@1.0.0

🔗 View in Sentry
```

---

## 📞 Дополнительная помощь

- Подробная инструкция: `SETUP_SENTRY_WEBHOOKS.md`
- Быстрая инструкция: `QUICK_WEBHOOK_SETUP.md`
- Автоматическая настройка: `setup_webhook_interactive.py`


