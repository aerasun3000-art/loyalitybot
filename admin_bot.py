...
async def show_b2b_deals_menu(callback_query: types.CallbackQuery):
    ...
    await callback_query.message.edit_text(
        ...
        "• Из этого фонда: 30% системе, 70% партнеру-источнику (БЕЗ MLM!)\n"
        "• Покупатель получает отдельный кэшбэк (указывается при создании сделки)\n\n"
        "**Пример:**\n"
        "Чек 10,000₽, продавец платит 10%:\n"
        "• Комиссионный фонд: 1,000₽\n"
        "• Система: 300₽ (30%)\n"
        "• Партнер-источник: 700₽ (70%)\n"
        "• Покупатель: кэшбэк по условиям сделки (например, 15% = 1,500₽)\n\n"
        "Выберите действие:",
        reply_markup=keyboard
    )
